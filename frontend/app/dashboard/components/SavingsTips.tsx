...
import type { Personality, SavingsTip } from "@/types";

interface SavingsTipsProps {
  tips: Array<string | SavingsTip>;
  personality?: Personality | null;
}

const SPENDING_TYPE_STYLE = {
  dengeli: {
...
const SPENDING_TYPE_LABEL_KEY: Record<string, string> = {
  dengeli: "savings.balancedSpender",
  tutumlu: "savings.frugalSpender",
  savruk: "savings.carelessSpender",
};
...
function renderTip(tip: string | SavingsTip, t: (k: string, p?: Record<string, unknown>) => string): string {
...
export default function SavingsTips({ tips, personality }: SavingsTipsProps) {
  ...
  const type = (personality?.spending_type as keyof typeof SPENDING_TYPE_STYLE) ?? "dengeli";
  const style = SPENDING_TYPE_STYLE[type] ?? SPENDING_TYPE_STYLE.dengeli;
  const label = t(SPENDING_TYPE_LABEL_KEY[type] ?? "savings.balancedSpender");
      ...
      {personality && (
        <motion.div {...fadeUp(0)} className={`card border-l-4 ${style.accent}`}>
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-semibold text-gray-800 dark:text-gray-100">{t("savings.profileTitle")}</h2>
      ...
      {tips.length > 0 && (
        <motion.div {...fadeUp(0.1)} className="card">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-semibold text-gray-800 dark:text-gray-100">{t("savings.tipsTitle")}</h2>
            <span className="ml-auto text-xs text-gray-400 dark:text-gray-500">{t("savings.tipsCount", { count: tips.length })}</span>
          </div>
          <div className="space-y-2">
            {tips.map((tip, i) => (
              ...
              >
                <span className={`w-6 h-6 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5 ${style.numberBg}`}>
                  {i + 1}
                </span>
                <p className="text-sm text-gray-600 dark:text-gray-300 flex-1 leading-snug">{renderTip(tip, t)}</p>
                {i === 0 && (
          ...
          >
            {t("savings.getProductSuggestion")}
